...
def lambda_n0(loc_n):
    rezult = complex(0, math.pi * (2 * loc_n + 1))
    return rezult
...
while i < count_pair:
    list_graf_Im_os.append((2 * i + 1) * math.pi)
...
while eps >= stop_eps:
    g = eps * lambda_(k)
    if(eps - step_eps < stop_eps) or (count_step % 20 == 0):
        list_graf_Re_prom.append(g)
        list_graf_Im_prom.append(0)
    count_step = count_step + 1
    list_graf_Re.append(g)
    list_graf_Im.append(0)
    eps = eps-step_eps
plt.plot(list_graf_Re, list_graf_Im, color=(0, 0, 1),linewidth = 1)
